...
class AbilityDifficulty(IterativeModel):
    ...
    def train(self, train_ts, val_ts, test_ts, S, Q, rate, iters, init, step_size):
        ...
        # Draw 1 samples for each (mean, std_dev) pair
        num_samples = 1
        epsilon = torch.randn(S, num_samples)  # samples from a standard normal (mean=0, std=1)
        bs = Ms.unsqueeze(1) + Ss.unsqueeze(1) * epsilon  # reparameterization
        ...
        for epoch in range(iters):
            params = {'bs': bs, 'bq': bq}     #  {'Ms': Ms, 'Ss': Ss, 'bq': bq}
    ...
    def calc_probit(self, data_ts, params):
        ### Draw from a Guassian for each student
        bs_data = torch.index_select(params['bs'], 0, data_ts[1])  ## params['bs'] => Bs = Mu_s+sigma_s*samples  
        ## Currently working out for EVERY student once, Now its a distribtion 
    ...
    def calc_nll(self, data_ts, params):
        probit_correct = self.calc_probit(data_ts, params)   ##To Draw M random samples##
